...
class LeadersOfCollageController extends Controller
{
    public function show()
    {
        $allHeaderImages = LeadersOfCollage::all();
        $data = [];
    ...
    public function update(Request $request)
    {
        $data = $request->validate([
            ...
            'title' => ['required', 'string'],
            'image' => ['nullable', 'image'],
            'id' => ['required', 'exists:leaders_of_collages,id']
        ]);
        $record = LeadersOfCollage::find($request->id);
